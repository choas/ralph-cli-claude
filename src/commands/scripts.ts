...
claude --permission-mode acceptEdits --dangerously-skip-permissions \\
  -p "@prd.json @progress.txt $PROMPT"
`;
...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  set +e
  claude --permission-mode acceptEdits --dangerously-skip-permissions \\
    -p "@prd.json @progress.txt $PROMPT" | tee "$TEMP_FILE"
  set -e
...
export async function scripts(_args: string[]): Promise<void> {
  ...
  // Check if config exists (means init was run)
  if (!existsSync(join(cwd, "ralph.config.json"))) {
