...
const DOCKER_COMPOSE = `# Ralph CLI Docker Compose
# Generated by ralph-cli
...
async function generateFiles(ralphDir: string, language: string): Promise<void> {
...
`);
  ...
  if (flag === "--build") {
    ...
    console.log(`Generating Docker files for language: ${config.language}\n`);
    await generateFiles(ralphDir, config.language);
...
Next steps:
  ...
  2. Build the image: ralph docker --build
  3. Run container:    ralph docker --run
